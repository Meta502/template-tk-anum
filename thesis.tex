...
% intinya ntar hitung lewat normcdf terus state kalau err > tol sehingga kurang akurat, prinsip si komposit kalau mau lebih akurat = naikin nilai partisinya tapi risikonya kompleksitas semakin rumit sehingga running time akan semakin lama.
...
\emph{Adaptive quadrature} merupakan metode aproksimasi fungsi integrasi yang rekursif dengan setiap kondisi yang belum memenuhi akan membagi \emph{problem} menjadi 2 buah \emph{sub-problem}. Perhitungan kompleksitas berikut ini akan dihitung dengan menghitung \emph{function call}. Untuk setiap pemanggilan fungsi di atas dilakukan 3 \emph{function call} untuk mendapatkan \emph{f(a), f(b), f(c)}. Untuk tiap pemecahan
